...
(defn create-world []
  (let [world (create-gl-world)
    ...
    (-> world
        ...
        ;;           (create-image "resources/insert-menu.svg" 726 675 -1 50))
...
(defn draw-2d! [world]
  ...
  ;;   (fill-rect! (make-color 70 70 70) x y (+ 20 w) (+ 10 h))
        ...
        world (if (and (< elapsed 200)
                ...
                (do
                  (println! "set pivot" elapsed (:press-point world)
                            [(:x event) (:y event)] (:press-time world)
                            (get-current-time))
...
(defn window-changed [world event]
  (println! "window changed" event)
  (let [{:keys [width height]} event]
    (-> world
        (recompute-viewport width height)
        ;; (place-box :insert-menu :wx 0.5 :ry -0.6)
        ;; (place-box :action-menu :rx 0.5 :ry 0.5)
        ;; (place-box :mode-menu :rx -0.5 :ry 0.5)
        )))
        
(set-thing! [:projection-matrix]
